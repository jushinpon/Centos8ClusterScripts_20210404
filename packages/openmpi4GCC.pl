...
my $UCXVer = "UCX-1.8";#***** the UCX version for openmpi
my $UCXPath = "/opt/$UCXVer";#UCX installation path
my $currentVer = "openmpi-4.0.5";#***** the latest version of this package
my $prefixPath = "/opt/$currentVer-$UCXVer";
system ("rm -rf $prefixPath");# remove the older directory first

my $URL = "https://download.open-mpi.org/release/open-mpi/v4.0/openmpi-4.0.5.tar.bz2";
my $Dir4download = "$packageDir/$currentVer-$UCXVer"."_download"; #the directory we download MPICH
...
if(! (-e "$Dir4download/$currentVer.tar.bz2")){die "No $currentVer downloaded";}# if no mpich file

## tar -xvzf XXX(package name), and then cd this new folder	
system("rm -rf  $currentVer");
system("tar -jxvf $currentVer.tar.bz2"); #$Ch =  Check
if ($?){die "tar -jxvf openmpi failed\n";} 
...
system("./configure --prefix=$prefixPath --with-ucx=$UCXPath --with-hwloc --enable-debug  --enable-openib-rdmacm --with-verbs");# --with-slurm=[/usr/local] VERBOSE=1 |tee 00mpich_configure.txt"); #./configure
...
system("make -j $thread4make VERBOSE=1 |tee 00openmpi_make.txt"); 
if($?){die "make $currentVer process failed!\n Reason $?:$!\n";}

system("make install VERBOSE=1 |tee 00openmpi_makeInstall.txt");
