...
my $URL = "https://github.com/advancesoftcorp/lammps.git";#url to download
#my $URL = "https://lammps.sandia.gov/tars/lammps.tar.gz";#url to download
my $Dir4download = "$packageDir/lammps_ocp"; #the directory we download Mpich
...
##lmp_mpi will be in src
	...
	my $test=`$getdat`;
	chomp $test;
	my $lmp_exe = "/opt/lammps_ocp"."/lmp_ocp";### make date information
	my $lmp_exeDir = "/opt/lammps_ocp/";### make date information
...
chdir("$Dir4download/lammps/src");
	...
	"voronoi","electrode","granular");#,"ML-OC20DRIVER"
	#for bigwind only
	...
	system ("make -j $thread4make serial");####**** check how many cores you may use for compiling lammps
	die;
    #print ""
