...
##c include path
#sub cinclude_setting {
#    my $attached_ld = shift;
#	my $cinclude_path = $ENV{'C_INCLUDE_PATH'};	
#	$ENV{'C_INCLUDE_PATH'} = "$attached_ld:$cinclude_path";		
#}
##c++ include path
#sub cppinclude_setting {
#    my $attached_ld = shift;
#	my $cppinclude_path = $ENV{'CPLUS_INCLUDE_PATH'};	
#	$ENV{'CPLUS_INCLUDE_PATH'} = "$attached_ld:$cppinclude_path";		
#}
...
#my $cpp_include = "/home/packages/deepMD/deepmd-kit/source/deepmd_include";#2023/01/27
#my $cpp_include = "/opt/anaconda3/envs/deepmd-cpuV203/include";#2023/01/27
#cppinclude_setting($cpp_include);
...
my $URL = "wget https://github.com/lammps/lammps/archive/stable_23Jun2022_update2.tar.gz";#url to download
#my $URL = "https://lammps.sandia.gov/tars/lammps.tar.gz";#url to download
my $Dir4download = "$packageDir/lammps_deepMD"; #the directory we download Mpich
my $currentPath = getcwd(); #get perl code path
my $lmp_path = "/home/packages/lammps_deepMD/lammps-23Jun2022";
#
#`rm -rf /home/packages/deepMD/deepmd-kit/source/deepmd_include`;
#
#die;
...
system("cmake -C ../cmake/presets/most.cmake -C ../cmake/presets/nolib.cmake -DCMAKE_INSTALL_PREFIX=$prefix ../cmake");
system("cmake -D PKG_PLUGIN=ON -D PKG_KSPACE=ON -D LAMMPS_INSTALL_RPATH=ON -D BUILD_SHARED_LIBS=yes -D CMAKE_INSTALL_LIBDIR=lib -D CMAKE_INSTALL_FULL_LIBDIR=/opt/deepmd_lammpslib/lib ../cmake");
system("make clean");
