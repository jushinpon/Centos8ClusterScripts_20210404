...
#$reboot_check = "yes";
#`cp ./openfoam.repo /etc/yum.repos.d/`;
#`sudo rpm --import https://download.copr.fedorainfracloud.org/results/openfoam/openfoam/pubkey.gpg`;
#system("dnf install openfoam -y");
#die;
my %nodes = (
    #161 => [33..38],#1,3,39..
    161 => [1..42],#1,3,39..
...
#use scp for ssh test
	system("scp -o ConnectTimeout=5 ./openfoam.repo root\@$nodename:/etc/yum.repos.d/");    
    if($?){
...
$pm->start and next;
   ...
   #install openfoam
   system("$cmd 'rpm --import https://download.copr.fedorainfracloud.org/results/openfoam/openfoam/pubkey.gpg && dnf install openfoam -y;dnf install libfabric -y' ");
